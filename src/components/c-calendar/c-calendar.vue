  ...
  <view>
    <view class="a-y-center a-flex-space-between">
      <view class="a-y-center a-mt">
        <view
          class="arrow-left c-iconfont icon-arrow-lift"
          @click="switchMonth(0)"
        ></view>
      ...
      <view class="a-y-center">
        <view
          class="opt a-y-center a-x-center a-background-blue"
        ...
        <view v-for="(column, columnIndex) in row" :key="columnIndex">
          <view
            ...
            :class="{
              ...
              'type--not-cur-month':
                !column.type.currentMonth && !column.type.flag,
            }"
          >
...
export default class CCalendar extends Vue {
  ...
  private buildVacationInfo(): void {
    ...
    const vacationDate = addDate(
      termStartData,
      0,
      0,
      (this.vacationWeek - 1) * 7
    );
  ...
  private buildTips(): void {
    const termTipsData: Array<string> = [
      this.term,
      this.weekCount.toString(),
      this.termStart,
    ];
  ...
  public buildCalendar(date: Date): void {
    ...
    const goBackDays = -(monthFirstDay.getDay() === 0
      ? 7
    ...
    calendar.forEach((row) =>
      row.forEach((item, index) => {
        const curWeekSerial: number =
          Math.floor(
            (timeDiff(safeDate(this.termStart), start, true).days + 1) / 7
          ) + 1;
        if (index === 0) {
          item.type.flag = true;
        ...
        item.type = {
          ...
          vacation:
            this.vacationWeek <= curWeekSerial &&
            curWeekSerial <= this.weekCount,
  ...
  public switchMonth(type: number): void {
    this.buildCalendar(
      addDate(safeDate(this.year, this.month - 1), 0, type === 0 ? -1 : 1)
    );
  }
